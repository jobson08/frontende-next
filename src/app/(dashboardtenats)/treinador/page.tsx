...
const TreinadorDashboardPage = () => {
  ...
  const [treinadorParaRemover, setTreinadorParaRemover] = useState<Treinador | null>(null);

  
  // Persistência do modo de visualização
  const [viewMode, setViewMode] = useState<"cards" | "table">(() => {
    const saved = localStorage.getItem("treinador-view-mode");
  ...
  useEffect(() => {
      localStorage.setItem("treinador-view-mode", viewMode);
  }, [viewMode]);

 const { data: treinador = [], isLoading, error } = useQuery<Treinador[]>({
    queryKey: ["treinador", searchTerm],
    queryFn: async () => {
   ...
   const deleteMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      queryClient.invalidateQueries({ queryKey: ["treinador"] });
      setTreinadorParaRemover(null);
    },
    onError: (err: any) => {
  ...
  //pesquisar
   const filtered = treinador.filter(r =>
  ...
  const confirmarRemocao = () => {
    if (treinadorParaRemover) {
      deleteMutation.mutate(treinadorParaRemover.id);
    }
  };
  ...
  if (isLoading) {
    return (
      <div className="flex min-h-screen items-center justify-center">
        ...
        <span className="ml-4 text-xl">Carregando treinador...</span>
      </div>
    );
  }
  ...
  if (error) {
    return (
      <div className="p-8 text-center text-red-600">
        ...
        <h2 className="text-2xl font-bold">Erro ao carregar Treinador</h2>
        <p className="mt-2">{(error as Error).message || "Tente novamente mais tarde"}</p>
        <Button className="mt-6" onClick={() => queryClient.refetchQueries({ queryKey: ["treinador"] })}>
          Tentar novamente
        </Button>
      </div>
    );
  ...
  return (
     ...
     <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold">Treinador</h1>
          <p className="text-gray-600">Gerencie todos os Treinador</p>
        </div>
        <div className="flex flex-wrap gap-3">
          <Button asChild className="bg-blue-600 hover:bg-blue-700">
            <Link href="/treinador/novo">
      ...
      <AlertDialog open={!!treinadorParaRemover} onOpenChange={() => setTreinadorParaRemover(null)}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Confirmar remoção?</AlertDialogTitle>
            <AlertDialogDescription>
              Tem certeza que deseja remover <strong>{treinadorParaRemover?.nome}</strong>?
              <br />
              Essa ação não pode ser desfeita.
            </AlertDialogDescription>
          </AlertDialogHeader>
